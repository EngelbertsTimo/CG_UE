...
public:
    ...
    const std::vector<glm::vec3>& getVertexNormals() const; // Normale der Punkte = Durschnitt der Normalen der Angrenzenden Normalen
    const std::vector<glm::vec3>& getVertexColors() const;
    const std::vector<glm::vec2>& getVertexTexCoords() const;

    const std::vector<unsigned int>& getTriangleIndices() const;

    const std::vector<glm::vec3>& getFaceNormals() const; // Normale des Dreiecks
